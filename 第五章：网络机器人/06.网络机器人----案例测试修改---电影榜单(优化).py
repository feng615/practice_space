...
# 获取电影上映年份
def get_all_movie_year(all_movie_year):
    movie_year= all_movie_year[0].strip() if all_movie_year else "" #（2026）
    #清洗数据，去除括号
    return movie_year.replace("(","").replace(")","")


def get_all_movie_datetime(all_movie_datetime):
    movie_datetime= all_movie_datetime[0].strip() if all_movie_datetime else ""#上映时间:2026-12-01（US）
    #清洗数据，获取上映时间
    return re.search(r"(\d{4}-\d{2}-\d{2})",movie_datetime).group()


def get_all_movie_time(all_movie_time):
    movie_time= all_movie_time[0].strip() if all_movie_time else ""#片时长:1h42m
    #清洗数据，获取时长
...
def get_movie_detail(movie_detail_url):
    ...
    movie_info ={
        ...
        "上映年份":get_all_movie_year(all_movie_year),
        "上映时间":get_all_movie_datetime(all_movie_datetime),
        "电影类型":",".join(all_movie_type) if all_movie_type else "",
        "电影时长":get_all_movie_time(all_movie_time),
...
def save_movies_info(all_movies_info):
    ...
    # csv文件保存
    with open("csv.data/movies1.csv", "w", encoding="UTF-8", newline="") as f:
